...
{
	const savedSearchName = candidateName.value;
...
if (savedSearchName) {
  ...
  chrome.storage.local.set({ [savedSearchName]: savedSearchData }, function() {
    console.log('Data saved [' + savedSearchName + '] to chrome.storage.local');
    
    // Store the search name in localStorage
    localStorage.setItem("savedSearchName", savedSearchName);
  });




  // save the final comment for Vincere  
const finalCommentValue = finalComment.value;
finalMessage_val = finalComment.value; // save this globally in script for use later with AI

 //localStorage.setItem("finalComment_storage", finalCommentValue);
 //console.log('Final comment saved to localStorage');

 chrome.storage.local.set({ ["finalComment_storage"]: finalCommentValue }, function() {
    console.log('Final Comment Saved saved [' + "finalComment_storage" + '] to chrome.storage.local');
...
function handleUserInput() {
	...
	const promptTemplate = 'Write a short email, not too formal, selling my candidate as a good fit for the job. If we know them, put his Location, VISA Status, Notice Period, and Desired Salary under the short paragraph. Here are some notes from a call with the candidate: Give me 2 results so i can chose the one i prefer.';
	
	const prompt = promptTemplate + ": " + finalMessage_val;  
	console.log(prompt);
  ...
  makeRequest(prompt)
    .then(response => {
	  ...
	  document.getElementById("AI_notesInput").hidden = false;
		document.getElementById("AI_notesInput").innerHTML = response;
...
document.getElementById('candidateAI_btn').addEventListener('click', handleUserInput);
if(localStorage.getItem('name') === "Zak")
{
	document.getElementById('candidateAI_btn').hidden = false;
}